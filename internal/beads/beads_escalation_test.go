...
// bdArgsStub installs a fake `bd` on PATH that appends every argument of every
// invocation to args.txt (one per line, blank line between invocations) and
// replies with responses[subcommand]. It returns a reader for the recorded args.
//
// A single-response stub is not enough for the multi-call paths below:
// ReescalateEscalation reads the bead with `show` before writing it with
// `update`, and feeding the update reply to the show would make the test fail
// for a reason unrelated to what it measures.
func bdArgsStub(t *testing.T, responses map[string]string) (argsPath string) {
	t.Helper()
	stubDir := t.TempDir()
	argsPath = filepath.Join(stubDir, "args.txt")

	var cases []string
	for sub, reply := range responses {
		cases = append(cases, "  "+sub+") printf '%s\\n' '"+reply+"' ;;")
	}
	slices.Sort(cases)

	// The subcommand is the first NON-FLAG argument, not $1: runWithStdin
	// prepends --allow-stale when bd advertises it, so args[0] is a flag on a
	// real invocation and a stub keyed on $1 answers every call from its
	// fallback branch.
	stubScript := `#!/bin/sh
for a in "$@"; do
  printf '%s\n' "$a" >> "` + argsPath + `"
...
// bdInvocations splits the recorded args file into one []string per invocation.
func bdInvocations(t *testing.T, argsPath string) [][]string {
	t.Helper()
...
// bdInvocationFor returns the recorded arguments of the first invocation of the
// named subcommand, so an assertion cannot accidentally read another call's
// flags. It identifies the subcommand the way the stub does: the first
// non-flag argument, since --allow-stale may be prepended.
func bdInvocationFor(t *testing.T, argsPath, sub string) []string {
	t.Helper()
	for _, args := range bdInvocations(t, argsPath) {
	...
	t.Fatalf("no `bd %s` invocation recorded; got %v", sub, bdInvocations(t, argsPath))
	return nil
}

// TestCreateEscalationBead_CarriesSeverityAsPriority is the acceptance test for
// gt-nhp's second defect: an escalation filed -s HIGH must read as a HIGH bead.
//
// It did not. hq-wisp-yro9 went in at HIGH about a live nuke hazard and `bd show`
// rendered it "[● P2 · OPEN]", because CreateEscalationBead wrote the severity as
// a LABEL only and let bd's default supply the priority. Measured on the hq
// store 2026-08-18, all 12 most recent escalation records sat at priority 2
// whatever their severity label said.
//
// The severity label alone is not a substitute: priority is the column generic
// readers render and, more importantly, the column the reaper's staleness sweep
// keys its P0/P1 exemption on — so a severity that never reached it also
// forfeited that protection.
func TestCreateEscalationBead_CarriesSeverityAsPriority(t *testing.T) {
	for severity, wantPriority := range map[string]string{
		...
		t.Run(severity, func(t *testing.T) {
			argsPath := bdArgsStub(t, map[string]string{
				"create": `{"id":"hq-wisp-rec1","title":"x","status":"open","type":"task","labels":["gt:escalation"]}`,
			})

			b := New(t.TempDir())
			if _, err := b.CreateEscalationBead("nuke hazard", &EscalationFields{
				Severity:    severity,
				EscalatedBy: "gastown/witness",
			}); err != nil {
				t.Fatalf("CreateEscalationBead: %v", err)
			...
			args := bdInvocationFor(t, argsPath, "create")
			if !slices.Contains(args, wantPriority) {
				t.Errorf("severity %q produced args %v, want %s — the severity label alone "+
					"leaves the record at bd's default P2", severity, args, wantPriority)
			}
			// The label must still be there: openEscalationCopies and
			// ListEscalationsBySeverity filter on it.
			if !slices.Contains(args, "--labels=severity:"+severity) {
				t.Errorf("severity label dropped from args %v", args)
			}
		})
	}
}

// An unrecognised severity must keep bd's default rather than silently landing
// at P0 — the same guard escalationPriority's ok return exists for.
func TestCreateEscalationBead_UnknownSeverityGetsNoPriority(t *testing.T) {
	argsPath := bdArgsStub(t, map[string]string{
		"create": `{"id":"hq-wisp-rec1","title":"x","status":"open","type":"task","labels":["gt:escalation"]}`,
	})

	b := New(t.TempDir())
	if _, err := b.CreateEscalationBead("odd one", &EscalationFields{Severity: "spicy"}); err != nil {
		t.Fatalf("CreateEscalationBead: %v", err)
	}
	for _, arg := range bdInvocationFor(t, argsPath, "create") {
		if strings.HasPrefix(arg, "--priority=") {
			t.Errorf("unknown severity produced %q; an unmapped severity must not be "+
				"promoted to a priority, least of all P0", arg)
		}
	}
}

// TestReescalateEscalation_MovesThePriorityWithTheSeverity: a re-escalation
// exists to make an ignored escalation louder. Bumping only the label and the
// description leaves the field readers sort by — and the field the staleness
// sweep exempts on — exactly where it was, which is the state that got the
// escalation ignored in the first place (gt-nhp).
func TestReescalateEscalation_MovesThePriorityWithTheSeverity(t *testing.T) {
	record := `[{"id":"hq-wisp-rec1","title":"disk filling","status":"open","priority":3,` +
		`"type":"task","labels":["gt:escalation","severity:low"],` +
		`"description":"disk filling\nseverity: low\nescalated_by: gastown/witness\n"}]`
	argsPath := bdArgsStub(t, map[string]string{"show": record, "update": `{}`})

	b := New(t.TempDir())
	result, err := b.ReescalateEscalation("hq-wisp-rec1", "gastown/deacon", 0)
	if err != nil {
		t.Fatalf("ReescalateEscalation: %v", err)
	}
	if result.NewSeverity != "medium" {
		t.Fatalf("NewSeverity = %q, want medium", result.NewSeverity)
	}

	args := bdInvocationFor(t, argsPath, "update")
	if !slices.Contains(args, "--priority=2") {
		t.Errorf("update args %v lack --priority=2 — low→medium bumped the label but left "+
			"the priority at P3", args)
	}
	// The label bump must survive alongside it.
	for _, want := range []string{"--add-label=severity:medium", "--remove-label=severity:low"} {
		if !slices.Contains(args, want) {
			t.Errorf("update args %v lack %s", args, want)
		}
	}
}
