...
// TestWrapErrorWithOutputSilentFailureIsDiagnosable covers gt-h7p. gt-k3h made
// bd's own output reach the operator; this is the case where there is no such
// output. A bare "bd create ...: exit status 1" reads as gt having discarded
// bd's stderr — that is what gt-h7p was filed as — so the error must state that
// bd said nothing, and name the database the write was aimed at, which is the
// only remaining lead and the reason the same command run by hand succeeds.
func TestWrapErrorWithOutputSilentFailureIsDiagnosable(t *testing.T) {
	b := New("/test")
	exitErr := errors.New("exit status 1")

	err := b.wrapErrorWithOutput(exitErr, "", "", []string{"create", "--json", "--ephemeral"})
	if err == nil {
		t.Fatal("wrapErrorWithOutput returned nil")
	}
	msg := err.Error()
	if !strings.Contains(msg, "bd wrote nothing to stdout or stderr") {
		t.Errorf("silent failure must say bd was silent, got: %v", err)
	}
	if !strings.Contains(msg, "beads_dir=") || !strings.Contains(msg, "cwd=/test") {
		t.Errorf("silent failure must name the target database and cwd, got: %v", err)
	}
	if !strings.Contains(msg, "exit status 1") {
		t.Errorf("silent failure must keep the underlying exit status, got: %v", err)
	}
}

// TestWrapErrorWithOutputKeepsBDOutputUnannotated guards the other direction:
// when bd did explain itself, the diagnostic note must not be appended. bd's
// own message is the answer; routing detail would only dilute it.
func TestWrapErrorWithOutputKeepsBDOutputUnannotated(t *testing.T) {
	b := New("/test")

	for _, tc := range []struct{ name, stderr, stdout string }{
		{"stderr", "dolt: connection refused", ""},
		{"stdout payload", "", `{"error":"database not initialized"}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			err := b.wrapErrorWithOutput(errors.New("exit status 1"), tc.stderr, tc.stdout, []string{"create"})
			if strings.Contains(err.Error(), "bd wrote nothing") {
				t.Fatalf("bd explained itself; error must not claim silence: %v", err)
			}
		})
	}
}

// TestAnnotateTimeout verifies the bd subprocess timeout is named. A bd killed
// by the deadline exits with both streams empty, so without this the caller
// sees only "signal: killed" and cannot tell a hung Dolt from a crash.
func TestAnnotateTimeout(t *testing.T) {
	...
	defer cancel()
	err := annotateTimeout(expired, killed, 60*time.Second)
	if !errors.Is(err, killed) {
		t.Fatalf("annotateTimeout must wrap the original error, got %v", err)
	}
	if !strings.Contains(err.Error(), "timed out after 1m0s") {
		t.Errorf("annotateTimeout should name the timeout, got: %v", err)
	}
	if !strings.Contains(err.Error(), "GT_BD_TIMEOUT_SEC") {
		t.Errorf("annotateTimeout should name the override knob, got: %v", err)
	}

	// A failure that is not a timeout must pass through untouched, or every
	// ordinary bd error would be reported as a hang.
	live, cancelLive := context.WithCancel(context.Background())
	defer cancelLive()
	if got := annotateTimeout(live, killed, 60*time.Second); got != killed {
		t.Errorf("non-timeout error was rewritten: %v", got)
	}
	if got := annotateTimeout(expired, nil, 60*time.Second); got != nil {
		t.Errorf("annotateTimeout(nil) = %v, want nil", got)
	}
}

// TestRunTimeoutSurfacesToCaller drives a bd that outlives the deadline through
// the real subprocess path. A killed bd leaves both streams empty, which is the
// gt-h7p failure shape — the caller must be told it was a timeout rather than
// left with "signal: killed".
func TestRunTimeoutSurfacesToCaller(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses POSIX shell fake bd")
	}

	binDir := t.TempDir()
	// Hang only on the real subcommand: the --allow-stale capability probe runs
	// bd too, and stalling that would time out the probe instead of the create.
	script := "#!/bin/sh\nfor a in \"$@\"; do [ \"$a\" = \"create\" ] && sleep 30; done\nexit 0\n"
	...
	started := time.Now()
	_, err := NewIsolated(t.TempDir()).Create(CreateOptions{Title: "Merge: gt-h7p", Priority: 1, Ephemeral: true})
	elapsed := time.Since(started)

	if err == nil {
		t.Fatal("Create succeeded against a bd that never returns")
	}
	if elapsed > 20*time.Second {
		t.Fatalf("Create took %s — the timeout did not fire", elapsed)
	}
