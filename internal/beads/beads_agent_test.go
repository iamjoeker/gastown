...
// TestListAgentBeadsUsesTownBeadsDir pins the gt-gkl regression: agent beads
// live only in the town database, so a rig-scoped client must still resolve
// them. Before the fix, ListAgentBeads queried the rig database, returned an
// empty set, and every caller (gt polecat list, capacity, mail resolution)
// read that absence as "cleanup_status missing" rather than "wrong database".
func TestListAgentBeadsUsesTownBeadsDir(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mock for bd")
	}
	...
	if err := WriteRoutes(townBeadsDir, []Route{{Prefix: "hq-", Path: "."}, {Prefix: "gt-", Path: "gastown"}}); err != nil {
		t.Fatalf("write routes: %v", err)
	}

	// The mock stands in for the real split: the town database holds the agent
	// bead, the rig database holds nothing.
	binDir := t.TempDir()
	logPath := filepath.Join(binDir, "bd.log")
	townAgents := `[{"id":"gt-gastown-polecat-chrome","title":"chrome","issue_type":"agent",` +
		`"labels":["gt:agent"],"status":"open",` +
...
exit 0
`, logPath, townBeadsDir, townAgents)
	scriptPath := filepath.Join(binDir, "bd")
	if err := os.WriteFile(scriptPath, []byte(script), 0755); err != nil {
		t.Fatalf("write mock bd: %v", err)
	}
	...
	agents, err := New(rigPath).ListAgentBeads()
	if err != nil {
		t.Fatalf("ListAgentBeads: %v", err)
	}
	if _, ok := agents["gt-gastown-polecat-chrome"]; !ok {
		t.Fatalf("rig-scoped ListAgentBeads did not resolve the town agent bead; got %v", agents)
	}
	...
	if logOutput := string(logBytes); strings.Contains(logOutput, "beads_dir="+rigBeadsDir) {
		t.Fatalf("ListAgentBeads queried the rig BEADS_DIR; log:\n%s", logOutput)
	}
}

// TestListAgentBeadsOutsideTownStaysLocal guards the fix from over-reaching:
// with no town root above the working directory there is nothing to resolve
// against, so the client must keep querying its own database.
func TestListAgentBeadsOutsideTownStaysLocal(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mock for bd")
	}
...
case "$cmd" in
  version) exit 0 ;;
  list) printf '[{"id":"local-agent","issue_type":"agent","labels":["gt:agent"],"status":"open"}]\n'; exit 0 ;;
  mol) exit 1 ;;
esac
exit 0
`, logPath)
	...
	agents, err := New(workDir).ListAgentBeads()
	if err != nil {
		t.Fatalf("ListAgentBeads: %v", err)
	}
	if _, ok := agents["local-agent"]; !ok {
