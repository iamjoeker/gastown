...
func pushedBranchFixture(t *testing.T) (string, string, string) {
	...
	if err := os.WriteFile(filepath.Join(localDir, "done.go"), []byte("package done\n"), 0644); err != nil {
		t.Fatalf("write done.go: %v", err)
	}
	mergeProofGit(t, localDir, "add", ".")
	mergeProofGit(t, localDir, "commit", "-m", "done: merged work")
	mergeProofGit(t, localDir, "push", "origin", branch)
	return localDir, branch, mergeProofGit(t, localDir, "rev-parse", "HEAD")
}
