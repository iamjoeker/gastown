...
func TestStaleBinaryInfo_Describe(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// command and can't afford a fetch), so its compare commit is a
			// local ref that can lag real main for hours. Describe must say so
		...
		{
			// A refresh that was attempted and failed still proved staleness
			// via local-lag containment; it should not get the "never
			// refreshed" caveat, which would misstate what happened.
