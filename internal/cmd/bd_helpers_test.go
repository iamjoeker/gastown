...
// newRoutedTownDir builds a town tree whose town-level .beads publishes
// routes.jsonl and whose rig has its own database with none, matching the
// layout in which gt-dno was reported.
func newRoutedTownDir(t *testing.T) string {
	t.Helper()
	townRoot := t.TempDir()
...
func TestBdCmd_WithRoutingRunsWhereRoutesLive(t *testing.T) {
	townRoot := newRoutedTownDir(t)
	rigDir := filepath.Join(townRoot, "gastown", "mayor", "rig")

	// A routing command issued from a rig context must still run where
	// routes.jsonl lives. bd only consults routes in the beads directory it
	// already resolved to, so leaving cwd in the rig silently drops routing and
	// reports every foreign-prefix bead as missing (gt-dno).
	cmd := BdCmd("show", "hq-abc", "--json").Dir(rigDir).WithRouting().Build()
	if cmd.Dir != townRoot {
		t.Errorf("routing command cwd = %q, want town root %q", cmd.Dir, townRoot)
	}

	// Routing means bd picks the database, so BEADS_DIR must stay unpinned.
	for _, e := range cmd.Env {
		if strings.HasPrefix(e, "BEADS_DIR=") {
			t.Errorf("routing command pinned %s, want BEADS_DIR unset so bd can route", e)
		}
	}
}

func TestBdCmd_WithoutRoutingKeepsCallerDir(t *testing.T) {
	townRoot := newRoutedTownDir(t)
	rigDir := filepath.Join(townRoot, "gastown", "mayor", "rig")

	// Pinned commands name their database explicitly and must not be relocated.
	cmd := BdCmd("show", "gt-abc", "--json").Dir(rigDir).Build()
	if cmd.Dir != rigDir {
		t.Errorf("pinned command cwd = %q, want caller dir %q", cmd.Dir, rigDir)
	}
}

func TestBdCmd_WithRoutingOutsideTownKeepsCallerDir(t *testing.T) {
	outside := t.TempDir()

	// No reachable routes.jsonl: relocating would change which database bd
	// tries first without buying any routing, so the caller's dir stands.
	cmd := BdCmd("show", "hq-abc", "--json").Dir(outside).WithRouting().Build()
	if cmd.Dir != outside {
		t.Errorf("routing command cwd = %q, want unchanged %q", cmd.Dir, outside)
	}
}
