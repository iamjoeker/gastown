...
// TestNudgeDeliveryOutcomes verifies the three outcomes a nudge can have
// (gt-lae6). Before this, nudgeWitness returned nothing at all, so no caller
// could distinguish "both paths delivered" from "both paths failed".
func TestNudgeDeliveryOutcomes(t *testing.T) {
	eventErr := fmt.Errorf("emit failed: disk full")
	sessionErr := fmt.Errorf("submit not verified: composer contains other text after Enter")
	...
	tests := []struct {
	...
	}{
		{
			...
			wantDelivered: true,
			wantDegraded:  false,
			wantSummary:   []string{"event emitted", "tmux nudge delivered"},
		},
		{
			// The reproduced case: probeComposerDirty gives up with no retry,
			// so only the event path reached the witness.
			name:          "event only",
			delivery:      nudgeDelivery{EventEmitted: true, SessionErr: sessionErr},
			wantDelivered: true,
			wantDegraded:  true,
			wantSummary:   []string{"event emitted", "tmux nudge failed", "composer contains other text"},
		},
		{
			name:          "tmux only",
			delivery:      nudgeDelivery{EventErr: eventErr, SessionNudged: true},
			wantDelivered: true,
			wantDegraded:  true,
			wantSummary:   []string{"event emit failed", "disk full", "tmux nudge delivered"},
		},
		{
			...
			wantDelivered: false,
			wantDegraded:  false,
			wantSummary:   []string{"event emit failed", "tmux nudge failed"},
		},
		{
			name:          "test hook stands in for delivery",
			delivery:      nudgeDelivery{TestHook: true},
			wantDelivered: true,
			wantDegraded:  false,
			wantSummary:   []string{"test hook"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.delivery.Delivered(); got != tt.wantDelivered {
				t.Errorf("Delivered() = %v, want %v", got, tt.wantDelivered)
			}
			if got := tt.delivery.Degraded(); got != tt.wantDegraded {
				t.Errorf("Degraded() = %v, want %v", got, tt.wantDegraded)
			}
			summary := tt.delivery.Summary()
...
// TestNudgeDeliverySummaryFlattensMultilineErrors keeps the summary printable
// on one console line even when a path fails with a multi-line error.
func TestNudgeDeliverySummaryFlattensMultilineErrors(t *testing.T) {
	d := nudgeDelivery{
		EventEmitted: true,
		SessionErr:   fmt.Errorf("nudge failed:\n  line two\n  line three"),
	}
	summary := d.Summary()
	if strings.Contains(summary, "\n") {
		t.Fatalf("Summary() = %q, want a single line", summary)
	}
	if !strings.Contains(summary, "line two line three") {
		t.Errorf("Summary() = %q, want flattened error text", summary)
	}
}

// TestNudgeWitnessTestHookReportsDelivery verifies nudgeWitness still logs
// through the test hook and now reports the outcome to its caller.
func TestNudgeWitnessTestHookReportsDelivery(t *testing.T) {
	setupSlingTestRegistry(t)
	logPath := filepath.Join(t.TempDir(), "nudge.log")
	t.Setenv("GT_TEST_NUDGE_LOG", logPath)

	d := nudgeWitness("gastown", "POLECAT_DONE institute exit=COMPLETED")

	if !d.TestHook {
		t.Errorf("nudgeWitness() TestHook = false, want true under GT_TEST_NUDGE_LOG")
	}
	if !d.Delivered() {
		t.Errorf("nudgeWitness() Delivered() = false, want true under the test hook")
	}
	logBytes, err := os.ReadFile(logPath)
	if err != nil {
		t.Fatalf("read log: %v", err)
