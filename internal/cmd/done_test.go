...
// TestSummarizeDoneErrors covers gt-h7p. gt done accumulated its failure
// reasons in doneErrors and then dropped them on the floor: the slice was
// appended to at eleven sites and read at none, so a witness holding
// MRFailed=true had no way to learn why, and the polecat's printed warning
// died with its terminal. The summary now rides the completion nudge, which
// constrains it to one bounded line.
func TestSummarizeDoneErrors(t *testing.T) {
	tests := []struct {
		name string
		errs []string
		want string
		...
		{
			name: "no errors yields no summary",
			errs: nil,
			want: "",
		},
		{
			name: "single error passes through",
			errs: []string{"MR bead creation failed: bd create: exit status 1"},
			want: "MR bead creation failed: bd create: exit status 1",
		},
		{
			name: "multiple failures are joined",
			errs: []string{"push failed: permission denied", "MR bead creation failed: exit status 1"},
			want: "push failed: permission denied; MR bead creation failed: exit status 1",
		},
		{
			name: "multi-line bd output is flattened",
			errs: []string{"MR bead creation failed: bd create:\n  dolt: connection refused\n  retrying\n"},
			want: "MR bead creation failed: bd create: dolt: connection refused retrying",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := summarizeDoneErrors(tt.errs); got != tt.want {
				t.Fatalf("summarizeDoneErrors = %q, want %q", got, tt.want)
			}
		})
	}
}

// TestSummarizeDoneErrorsIsNudgeSafe pins the two invariants the delivery
// channel imposes. A newline would submit the tmux composer mid-message and
// break the single-line "message=" channel event; an unbounded bd error would
// swamp both.
func TestSummarizeDoneErrorsIsNudgeSafe(t *testing.T) {
	huge := strings.Repeat("dolt refused the connection ", 200)
	got := summarizeDoneErrors([]string{"MR bead creation failed:\n" + huge})

	if strings.ContainsAny(got, "\n\r") {
		t.Errorf("summary must be one line, got %q", got)
	}
	if len(got) > maxDoneErrorSummary+len("…(truncated)") {
		t.Errorf("summary length = %d, want bounded near %d", len(got), maxDoneErrorSummary)
	}
	if !strings.Contains(got, "truncated") {
		t.Errorf("summary should mark truncation, got %q", got)
	}
	// The reason must survive truncation, not just the prefix boilerplate.
	if !strings.Contains(got, "dolt refused the connection") {
		t.Errorf("summary dropped the failure reason: %q", got)
	}
}

// TestDoneMessageWithErrorsStillParses guards the witness side: POLECAT_DONE is
// matched by prefix and polecat name, so appending failure detail must not stop
// the completion from being recognized.
func TestDoneMessageWithErrorsStillParses(t *testing.T) {
	message := fmt.Sprintf("POLECAT_DONE %s exit=%s", "settler", ExitCompleted)
	if reasons := summarizeDoneErrors([]string{"MR bead creation failed: exit status 1"}); reasons != "" {
		message += " errors=" + reasons
	}

	match := patternPolecatDone.FindStringSubmatch(message)
	if match == nil {
		t.Fatalf("witness no longer recognizes the completion message: %q", message)
	}
	if match[1] != "settler" {
		t.Fatalf("parsed polecat name = %q, want settler", match[1])
	}
	if !strings.Contains(message, "MR bead creation failed") {
		t.Fatalf("completion message lost the failure reason: %q", message)
	}
}

// TestWitnessCheckpointValue verifies the witness-notified checkpoint records
// what actually happened (gt-lae6). It used to be a hardcoded "ok", written
// before the nudge it claimed to describe was even attempted.
func TestWitnessCheckpointValue(t *testing.T) {
	sessionErr := errors.New("submit not verified: composer contains other text after Enter")
	eventErr := errors.New("emit failed")
	...
	tests := []struct {
		...
		{"both paths", nudgeDelivery{EventEmitted: true, SessionNudged: true}, "ok"},
		{"event only", nudgeDelivery{EventEmitted: true, SessionErr: sessionErr}, "degraded"},
		{"tmux only", nudgeDelivery{EventErr: eventErr, SessionNudged: true}, "degraded"},
		{"neither", nudgeDelivery{EventErr: eventErr, SessionErr: sessionErr}, "failed"},
		{"test hook", nudgeDelivery{TestHook: true}, "ok"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := witnessCheckpointValue(tt.delivery)
			if got != tt.want {
				t.Errorf("witnessCheckpointValue() = %q, want %q", got, tt.want)
			}
			// The label is done-cp:<stage>:<value>:<ts> and readDoneCheckpoints
			// splits on ':' — a value with a colon in it would be truncated.
			if strings.ContainsAny(got, ": \n") {
				t.Errorf("witnessCheckpointValue() = %q, want a single ':'-free token", got)
			}
		})
	}
}

// TestWitnessCheckpointValueRoundTrips confirms each checkpoint value survives
// the label encode/decode used by readDoneCheckpoints.
func TestWitnessCheckpointValueRoundTrips(t *testing.T) {
	for _, value := range []string{"ok", "degraded", "failed"} {
		label := fmt.Sprintf("done-cp:%s:%s:%d", CheckpointWitnessNotified, value, time.Now().Unix())
		parts := strings.SplitN(label, ":", 4)
		if len(parts) != 4 {
			t.Fatalf("label %q split into %d parts, want 4", label, len(parts))
		}
		if parts[2] != value {
			t.Errorf("decoded value = %q, want %q", parts[2], value)
		}
	}
}

// TestWitnessNotifyResult verifies gt done reports the real delivery outcome
// instead of the unconditional "✓ Witness notified" line (gt-lae6).
func TestWitnessNotifyResult(t *testing.T) {
	sessionErr := errors.New("submit not verified: composer contains other text after Enter")
	eventErr := errors.New("emit failed")
	...
	tests := []struct {
		...
		{
			name:       "both paths delivered",
			delivery:   nudgeDelivery{EventEmitted: true, SessionNudged: true},
			wantSymbol: "✓",
			wantParts:  []string{"Witness notified of DEFERRED"},
		},
		{
			name:       "degraded names the failed path",
			delivery:   nudgeDelivery{EventEmitted: true, SessionErr: sessionErr},
			wantSymbol: "⚠",
			wantParts:  []string{"degraded", "event emitted", "tmux nudge failed"},
		},
		{
			name:       "total failure is not reported as success",
			delivery:   nudgeDelivery{EventErr: eventErr, SessionErr: sessionErr},
			wantSymbol: "✗",
			wantParts:  []string{"NOT notified", "every delivery path failed"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			symbol, line := witnessNotifyResult("DEFERRED", tt.delivery)
			...
			if !tt.delivery.Delivered() && strings.Contains(line, "✓") {
				t.Errorf("line = %q, want no success marker when nothing was delivered", line)
			}
		})
	}
}
