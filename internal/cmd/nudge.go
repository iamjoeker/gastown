...
func deliverNudge(t *tmux.Tmux, sessionName, message, sender string) error {
	// Test hook: log the nudge instead of delivering through real tmux/queue
	// transport. Prevents test-suite runs from delivering "test" messages to
	// live agents (mayor reported recurring synthetic nudges traced to
	// nudge_test.go invocations). Mirrors nudgeWitness/nudgeRefinery.
	if logPath, inTest := testNudgeHook(); inTest {
		writeTestNudgeLog(logPath, fmt.Sprintf("nudge:%s:%s:%s\n", sessionName, sender, message))
		return nil
	}
