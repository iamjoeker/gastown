...
type fakeWitnessIssueShower struct {
	issue *beads.Issue
	err   error
}

func (f fakeWitnessIssueShower) Show(string) (*beads.Issue, error) { return f.issue, f.err }

// The witness's slot-reuse decision reads the same hook slot check-recovery
// does, and blocked on it the same way. A slot the issue store has released
// must not hold a slot shut (gt-dh3d).
func TestWitnessHookSlotState(t *testing.T) {
	const assignee = "gastown/polecats/synth"

	tests := []struct {
		name         string
		bd           witnessIssueShower
		hookBead     string
		wantTerminal bool
		wantSafe     bool
	}{
		{name: "no hook slot", bd: fakeWitnessIssueShower{}, hookBead: ""},
		{
			name:         "closed work",
			bd:           fakeWitnessIssueShower{issue: &beads.Issue{Status: "closed", Assignee: assignee}},
			hookBead:     "gt-work",
			wantTerminal: true,
			wantSafe:     true,
		},
		{
			name:     "reopened and unassigned is not this polecat's hook",
			bd:       fakeWitnessIssueShower{issue: &beads.Issue{Status: "open"}},
			hookBead: "gt-2uqy",
			wantSafe: true,
		},
		{
			name:     "genuinely hooked still blocks",
			bd:       fakeWitnessIssueShower{issue: &beads.Issue{Status: "hooked", Assignee: assignee}},
			hookBead: "gt-work",
		},
		{
			// Silence establishes nothing, so it cannot clear a slot.
			name:     "lookup error blocks",
			bd:       fakeWitnessIssueShower{err: errors.New("bd exploded")},
			hookBead: "gt-work",
		},
		{
			name:     "missing bead blocks",
			bd:       fakeWitnessIssueShower{},
			hookBead: "gt-work",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			terminal, safe := witnessHookSlotState(tt.bd, tt.hookBead, assignee)
			if terminal != tt.wantTerminal || safe != tt.wantSafe {
				t.Fatalf("witnessHookSlotState() = (terminal %v, safe %v), want (%v, %v)",
					terminal, safe, tt.wantTerminal, tt.wantSafe)
			}
		})
	}
}
