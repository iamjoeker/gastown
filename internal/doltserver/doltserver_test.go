...
// TestRefreshPIDStateFromLiveInfoRestartResetsStartedAt covers gt-pdd: when
// something other than 'gt dolt start' restarts Dolt, the live PID changes but
// the recorded start time used to be left alone, so 'gt dolt status' reported
// uptime counted from a process that had already died.
func TestRefreshPIDStateFromLiveInfoRestartResetsStartedAt(t *testing.T) {
	townRoot := t.TempDir()
	config := DefaultConfig(townRoot)
	...
	// This test binary stands in for the restarted server: a live PID that
	// differs from the recorded one.
	livePID := os.Getpid()
	if _, err := refreshPIDStateFromLiveInfo(townRoot, config, livePID); err != nil {
		t.Fatalf("refreshPIDStateFromLiveInfo: %v", err)
	}
	...
	if state.PID != livePID {
		t.Fatalf("state PID = %d, want %d", state.PID, livePID)
	}
	if state.StartedAt.Equal(stale) {
		t.Fatal("start time still points at the dead process — uptime would be overstated")
	}
	if want, ok := ProcessStartTime(livePID); ok && !state.StartedAt.Equal(want) {
		t.Fatalf("start time = %v, want the live process's %v", state.StartedAt, want)
	}
}

// TestRefreshPIDStateFromLiveInfoKeepsStartedAtWhenPIDUnchanged guards the
// other direction: a server that never restarted keeps the launch time gt
// recorded, rather than being rewritten on every status call.
func TestRefreshPIDStateFromLiveInfoKeepsStartedAtWhenPIDUnchanged(t *testing.T) {
	townRoot := t.TempDir()
	config := DefaultConfig(townRoot)

	livePID := os.Getpid()
	recorded := time.Now().Add(-90 * time.Minute).Truncate(time.Second)
	if err := SaveState(townRoot, &State{
		Running:   true,
		PID:       livePID,
		Port:      config.Port,
	...
	if !state.StartedAt.Equal(recorded) {
		t.Fatalf("start time = %v, want the unchanged %v", state.StartedAt, recorded)
	}
}
