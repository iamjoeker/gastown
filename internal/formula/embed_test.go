...
// TestCheckFormulaHealth_ModifiedShadowingNewerEmbedded covers the case that
// makes a shipped formula fix invisible to an existing town: the file was
// customized locally AND the embedded copy has moved since it was installed.
// UpdateFormulas skips modified files by design, so the town keeps running its
// local edit; without the drift flag the health report calls that "modified"
// and says nothing about the fix it is shadowing (gt-0sq).
func TestCheckFormulaHealth_ModifiedShadowingNewerEmbedded(t *testing.T) {
	const target = "mol-deacon-patrol.formula.toml"

	// Control: a local edit made against the CURRENT embedded copy is modified
	// but not drifted — there is no newer default behind it.
	t.Run("no drift when install is current", func(t *testing.T) {
		tmpDir := t.TempDir()
		...
		formulasDir := filepath.Join(tmpDir, ".beads", "formulas")
		if err := os.WriteFile(filepath.Join(formulasDir, target), []byte("# local edit\n"), 0644); err != nil {
			t.Fatal(err)
		}
		...
		if report.ModifiedDrift != 0 {
			t.Errorf("ModifiedDrift = %d, want 0", report.ModifiedDrift)
		}
	})

	t.Run("drift when embedded moved after install", func(t *testing.T) {
		tmpDir := t.TempDir()
		if _, err := ProvisionFormulas(tmpDir); err != nil {
			t.Fatalf("ProvisionFormulas() error: %v", err)
		}
		...
		// Local edit shadowing the embedded copy...
		if err := os.WriteFile(filepath.Join(formulasDir, target), []byte("# local edit\n"), 0644); err != nil {
			t.Fatal(err)
		}
		// ...installed from an older embedded copy than the one shipping now.
		staleHash := sha256.Sum256([]byte("# what shipped when this town installed\n"))
		installed.Formulas[target] = hex.EncodeToString(staleHash[:])
		if err := saveInstalledRecord(formulasDir, installed); err != nil {
			t.Fatal(err)
		}
		...
		if report.ModifiedDrift != 1 {
			t.Errorf("ModifiedDrift = %d, want 1", report.ModifiedDrift)
		}
		...
		for _, f := range report.Formulas {
			...
			found = true
			if f.Status != "modified" {
				t.Errorf("%s status = %q, want %q", target, f.Status, "modified")
			}
			if !f.EmbeddedChanged {
				t.Errorf("%s EmbeddedChanged = false, want true", target)
			}
		}
		if !found {
			t.Errorf("%s not found in report", target)
		}
	})
}
...
// TestUpdateFormulasDetailed_NamesSkippedAndDrifted covers the report that makes
// a refusal audible. UpdateFormulas is right to leave a customized formula alone,
// but returning only a count of "skipped" cannot distinguish a harmless local edit
// from one pinning the town behind a newer shipped default (gt-bxu).
func TestUpdateFormulasDetailed_NamesSkippedAndDrifted(t *testing.T) {
	const target = "mol-deacon-patrol.formula.toml"

	// Control: edited against the current embedded copy — skipped, not drifted.
	t.Run("modified without drift", func(t *testing.T) {
		tmpDir := t.TempDir()
		if _, err := ProvisionFormulas(tmpDir); err != nil {
			t.Fatalf("ProvisionFormulas() error: %v", err)
		}
		...
		report, err := UpdateFormulasDetailed(tmpDir)
		if err != nil {
			t.Fatalf("UpdateFormulasDetailed() error: %v", err)
		}
		if len(report.Skipped) != 1 || report.Skipped[0] != target {
			t.Errorf("Skipped = %v, want [%s]", report.Skipped, target)
		}
		if len(report.Drifted) != 0 {
			t.Errorf("Drifted = %v, want empty", report.Drifted)
		}
	})

	t.Run("modified with drift", func(t *testing.T) {
		tmpDir := t.TempDir()
		if _, err := ProvisionFormulas(tmpDir); err != nil {
			t.Fatalf("ProvisionFormulas() error: %v", err)
		}
		...
		if len(report.Drifted) != 1 || report.Drifted[0] != target {
			t.Errorf("Drifted = %v, want [%s]", report.Drifted, target)
		}

		// Re-running must not clear it: that permanence is the whole point.
		again, err := UpdateFormulasDetailed(tmpDir)
		if err != nil {
			t.Fatalf("UpdateFormulasDetailed() second call error: %v", err)
		}
		if len(again.Drifted) != 1 {
			t.Errorf("Drifted after second run = %v, want it to persist", again.Drifted)
		}
	})
}
...
// TestPours verifies the pour lookup sling uses to decide whether bd should
// materialize a formula's steps (gt-pzx). It answers for the copy that
// executes, so it follows the same rig > town > embedded precedence.
func TestPours(t *testing.T) {
	t.Run("embedded formula without a pour key does not pour", func(t *testing.T) {
		pours, err := Pours("mol-polecat-work", "", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if pours {
			t.Error("mol-polecat-work declares no pour, want false")
		}
	})

	t.Run("embedded formula with pour = true pours", func(t *testing.T) {
		pours, err := Pours("beads-release", "", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if !pours {
			t.Error("beads-release declares pour = true, want true")
		}
	})

	t.Run("disk copy shadows embedded", func(t *testing.T) {
		tmpDir := t.TempDir()
		rigFormulasDir := filepath.Join(tmpDir, "myrig", ".beads", "formulas")
		if err := os.MkdirAll(rigFormulasDir, 0755); err != nil {
			t.Fatal(err)
		...
		pours, err := Pours("mol-polecat-work", tmpDir, "myrig")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if !pours {
			t.Error("rig copy declares pour = true, want true — the disk copy is what runs")
		}
	})

	t.Run("reads a formula file by path", func(t *testing.T) {
		// Sling extracts the embedded formula to a temp file when a rig has no
		// formulas on disk, and hands bd that path.
		path := filepath.Join(t.TempDir(), "mol-temp.formula.toml")
		if err := os.WriteFile(path, []byte("formula = \"mol-temp\"\npour = true\n"), 0644); err != nil {
			t.Fatal(err)
		}
		pours, err := Pours(path, "", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if !pours {
			t.Error("formula file declares pour = true, want true")
		}
	})

	t.Run("returns error when the formula resolves nowhere", func(t *testing.T) {
		if _, err := Pours("mol-does-not-exist", "", ""); err == nil {
			t.Error("expected error for non-existent formula")
		}
	})
}
