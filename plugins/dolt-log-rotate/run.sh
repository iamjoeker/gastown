...
# so `$(gt town root 2>/dev/null)` assigned help text to TOWN_ROOT and LOG_DIR
# became a nonsense path — silently, and the plugin then reported "no log file,
# nothing to do" forever (gt-cr2). The -d check keeps that failure loud even
# against a gt binary predating the fix.
